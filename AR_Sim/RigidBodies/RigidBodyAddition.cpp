...
void ParticleSystem::initBanana(glm::vec3 pos, glm::vec3 vel, glm::vec3 ang, float scale)
{
	std::string line;
	std::string fileName("Data/OBJparticles/banana/banana_");
	if (abs(scale - 1.0 < 0.01))
		fileName += "1_0";
	else if (abs(scale - 1.5 < 0.01))
		fileName += "1_5";
	...
	if (myfile.is_open())
	{
		...
		std::cout << "Banana object has " << particles << " particles" << std::endl;
		bananaParticles = particles;
		firstBananaIndex = numRigidBodies;
		...
		{
			m_hPos[4 * i] -= cm.x;
			m_hPos[4 * i + 1] -= cm.y + 3*m_params.particleRadius;
			m_hPos[4 * i + 2] -= cm.z;
			m_hPos[4 * i + 3] = 0.f;
		...
		test /= (float)particles;
		std::cout << "Obj particles banana corrected center of mass: (" << test.x << ", " << test.y << ", " << test.z << ")" << std::endl;
		if (!initializedNow)
			m_numParticles += particles;
		inertiaTensor = glm::inverse(inertiaTensor);
		std::cout << "Banana inverse inertia tensor: " << std::endl;
		for (int row = 0; row < 3; row++)
		{
			for (int col = 0; col < 3; col++)
				std::cout << inertiaTensor[row][col] << " ";
...
void ParticleSystem::addBanana(glm::vec3 pos, glm::vec3 vel, glm::vec3 ang, float scale)
{
	...
	unmapGLBufferObject(m_cuda_posvbo_resource);
	int start = m_numParticles;
	m_numParticles += bananaParticles;
	int particles = bananaParticles;
	std::cout << "Number of particles after newest addition: " << m_numParticles << std::endl;
	//reallocate client (GPU) memory to fit new data
	float *newRelativePos;
	checkCudaErrors(cudaMalloc((void**)&newRelativePos, 4 * sizeof(float) * particles));
	checkCudaErrors(cudaMemcpy(newRelativePos, bananaRelativePositions, 4 * sizeof(float) * particles, cudaMemcpyDeviceToDevice));
	...
	glm::mat3 *newInverseInertia;
	checkCudaErrors(cudaMalloc((void**)&newInverseInertia, sizeof(glm::mat3)));
	checkCudaErrors(cudaMemcpy(newInverseInertia, &rbInertia[firstBananaIndex], sizeof(glm::mat3), cudaMemcpyDeviceToDevice));
...
void ParticleSystem::initObject(glm::vec3 pos, glm::vec3 vel, glm::vec3 ang, float scale, const char* modelName, int objectType)
{
	...
	if (myfile.is_open())
	{
		...
		/*if (!strcmp(modelName, "banana"))
		{
			inertiaTensor[0][0] = 0.000008;
			inertiaTensor[0][1] = 0.000007;
			inertiaTensor[0][2] = 0.000000;

			inertiaTensor[1][0] = 0.000007;
			inertiaTensor[1][1] = 0.000023;
			inertiaTensor[1][2] = 0.000000;

			inertiaTensor[2][0] = 0.000000;
			inertiaTensor[2][1] = 0.000000;
			inertiaTensor[2][2] = 0.000027;
		}*/
